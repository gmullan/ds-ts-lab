...
const myFootballBuddies = makeBuddyList(
  "Football team",
  [colleagueV1, friends[0], colleagueV2],
  colleagueV1
)

const myBandBuddies = makeBuddyList(
    "Band name",
    [colleagueV1, friends[1]]
    // No administrator
  )
...
function getBuddyListFriends(list: BuddyList): Friend[] {
  const result = list.members.reduce(
    (friends: Friend[], buddy: Buddy) => {
      if ("phone" in buddy) {
        friends.push(buddy);
      }
      return friends;
